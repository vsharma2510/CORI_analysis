...
int main(int argc, char* argv[]){
  ...
  TString tex_file_name = Form("/global/homes/v/vsharma2/Analysis_code/Miscalibration/Output/Cal_plots_images/ds%d/Alpha/ds%d_alpha.txt",dataset,dataset);
  ...
  for(it=channel.begin()+1;it!=channel.end();it++){
    ...
    if(fitFunHandleC.IsValid()){
      ...
      fit_C->GetYaxis()->SetTitleOffset(1.45);
      fit_C->Draw();
    }
	...
	gr2_H->GetXaxis()->SetTitle("Stabilized Amplitude (heaterTGS)");
	gr2_H->GetYaxis()->SetTitle("Counts");
	gr2_H->SetTitle(Form("Fit of peak for energy %d keV",temp_energy_H));
	...
	gr_H->GetXaxis()->SetTitle("Stabilized Amplitude (heater_TGS)");
	gr_H->GetYaxis()->SetTitle("Counts");
	gr_H->SetTitle(Form("Fit of peak for energy %d keV",temp_energy_H));
	gr_H->Draw("SAME");
	gPeakH.Clear();
      }
	...
	hSpectrum_H->SetTitle(Form("Stabilized amplitude spectrum for ds%d ch %d (heater_TGS)",dataset,*it));
	...
	cout<<"spectHandle uses "<<temp_energy_H<<" with histogram range of "<<range_lower_H<<" "<<range_upper_H<<endl;
      ...
      output_file_name_heater = Form("/global/homes/v/vsharma2/Analysis_code/Miscalibration/Output/Cal_plots_images/ds%d/Alpha/cal_plot_ds%d_ch%d_%d_keV_heaterTGS.pdf",dataset,dataset,(*it),temp_energy_H);
      myCan_H->Update();
	...
	gr2_C->SetTitle(Form("Fit of peak for energy %d keV",temp_energy_C));
	...
	gr_C->GetXaxis()->SetTitle("Stabilized Amplitude (calibration_TGS)");
	gr_C->GetYaxis()->SetTitle("Counts");
	gr_C->SetTitle(Form("Fit of peak for energy %d keV",temp_energy_C));
	gr_C->Draw("SAME");
	gPeakC.Clear();
      }
	...
	hSpectrum_C->SetTitle(Form("Stabilized amplitude spectrum for ds%d ch %d (calibration_TGS)",dataset,*it));
	hSpectrum_C->SetXTitle("Stabilized Amplitude (calibrationTGS)");
	...
	cout<<"spectHandleC uses "<<temp_energy_C<<" with histogram range of "<<range_lower_C<<" "<<range_upper_C<<endl;
      ...
      output_file_name_cal = Form("/global/homes/v/vsharma2/Analysis_code/Miscalibration/Output/Cal_plots_images/ds%d/Alpha/cal_plot_ds%d_ch%d_%d_keV_calibrationTGS.pdf",dataset,dataset,(*it),temp_energy_C);
      myCan_C->Update();
